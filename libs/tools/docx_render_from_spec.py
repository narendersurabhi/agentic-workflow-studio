    ...
    registry.register(
        Tool(
            spec=ToolSpec(
                ...
                description="Render an ATS-friendly .docx document from a DocumentSpec JSON",
                usage_guidance=(
                    "Provide path (required relative .docx filename). document_spec is "
                    "resolved from memory (document_spec:latest) unless explicitly provided, along with "
            ...
            handler=_docx_render_from_spec,
        )
    )


def _docx_render_from_spec(payload: Dict[str, Any]) -> Dict[str, Any]:
